...
public static class Catbot
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        foreach (var user in users.ToList())
        {
            ...
            if (user.IsCooldown)
            {
                Log.Log(LogLevel.Information, "User {0} is Cooldown", user.DiscordID);
                
                var newUser = new UserModel
                {
                    DiscordID = user.DiscordID,
                    PatAmount = user.PatAmount,
                    MaxPats = user.MaxPats,
                    MinPats = user.MinPats,
                    IsCooldown = false,
                    TimeLeft = user.TimeLeft,
                };
                
                await collection.ReplaceOneAsync(u => u.DiscordID == user.DiscordID, newUser);
            }
        }
